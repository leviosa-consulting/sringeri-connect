...
export default function ServiceIcon({ title, icon: Icon, color, isExternal, url, onClick }: ServiceIconProps) {
  ...
  return (
    ...
    >
      <div className={cn(
        "relative p-4 rounded-2xl transition-all duration-300 group-hover:scale-110 group-active:scale-95 shadow-sm",
        color
      )}>
        <Icon className="h-6 w-6" />
        {isExternal && (
          <ExternalLink className="absolute -top-1 -right-1 h-3 w-3 text-muted-foreground bg-white rounded-full p-0.5" />
        )}
      </div>
